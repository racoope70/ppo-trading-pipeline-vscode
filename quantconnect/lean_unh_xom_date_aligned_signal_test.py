...
class UnhXomObjectStoreSignalSmokeTest(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # Date-aligned test: stale-signal validation should be active.
        ...
        self.max_signal_age = timedelta(days=10)
        ...
        self.debug(f"Bypass stale check: {self.bypass_stale_check}")
    ...
    def on_data(self, data: Slice):
        ...
        for ticker, symbol in self.symbols.items():
            ...
            self.debug(f"{self.time} | {ticker} | SUBMIT set_holdings({ticker}, {target_weight:.4f})")
            self.set_holdings(symbol, target_weight)

    def _load_signal_payload_from_object_store(self) -> str:
        key = "live_signals_date_aligned_feb10.json"
    ...
    def _signal_is_valid(self, signal_row: dict) -> bool:
        ...
        self.debug(f"{self.time} | signal_time={signal_time} | signal_age={signal_age}")
